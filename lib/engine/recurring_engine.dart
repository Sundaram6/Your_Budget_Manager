...
class RecurringEngine {
  ...
  /// 1. processDueRecurring() — static async, call on app startup:
  /// - Query recurring_transactions WHERE is_active=1 AND next_due_date <= today
  /// - For each, call _processSingle()
  ...
  static Future<int> processDueRecurring({DateTime? referenceDate}) async {
    ...
    final dueTransactions = await DatabaseHelper.instance.getDueRecurringTransactions(todayStr);
  ...
  /// 2. _processSingle(RecurringTransactionModel rt, DateTime referenceDate):
  /// - Loop: while currentDue <= referenceDate:
  ///   - If endDate exists and currentDue > endDate, break
  ...
  ///     - isRecurring = true, recurringId = rt.id
  ///   - Insert into transactions table
  ///   - Calculate nextDue using _calculateNextDue()
  ///   - Update recurring_transactions: next_due_date, last_generated_date, updated_at
  ///   - Increment count
  ...
  /// 3. _calculateNextDue(DateTime from, RecurringTransactionModel rt):
  /// - daily: +1 day
  /// - weekly: +7 days
  /// - biweekly: +14 days
  /// - monthly: DateTime(year, month+1, day)
  /// - yearly: DateTime(year+1, month, day)
  /// - custom: +intervalDays
  static DateTime _calculateNextDue(DateTime from, RecurringTransactionModel rt) {
  ...
  static DateTime _addMonths(DateTime from, int months) {
    ...
    final targetDay = from.day > daysInTargetMonth ? daysInTargetMonth : from.day;
    return DateTime(targetYear, targetMonth, targetDay, from.hour, from.minute, from.second);
  }
  ...
  static DateTime _addYears(DateTime from, int years) {
    ...
    final targetDay = from.day > daysInTargetMonth ? daysInTargetMonth : from.day;
    return DateTime(targetYear, from.month, targetDay, from.hour, from.minute, from.second);
  }

  /// 4. forceGenerate(String recurringId) — manual trigger for "Generate Now" button.
  static Future<int> forceGenerate(String recurringId) async {
    final rt = await DatabaseHelper.instance.getRecurringTransactionById(recurringId);
    if (rt == null) return 0;
